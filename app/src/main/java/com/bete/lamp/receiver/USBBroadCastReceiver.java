    ...
    public void onReceive(Context context, Intent intent) {
        ...
        switch (action) {
            ...
            case UsbManager.ACTION_USB_DEVICE_ATTACHED://接收到存储设备插入广播
                ...
                if (device_add != null) {
                    Toast.makeText(context, "device_add", Toast.LENGTH_SHORT).show();
                    if (usbListener != null) {
                        usbListener.insertUsb(device_add);
                    }
                }
                Log.d(TAG, "onReceive: device_add");
                break;
            case UsbManager.ACTION_USB_DEVICE_DETACHED:
                //接收到存储设备拔出广播
                UsbDevice device_remove = intent.getParcelableExtra(UsbManager.EXTRA_DEVICE);
                if (device_remove != null) {
                    Toast.makeText(context, "device_remove", Toast.LENGTH_SHORT).show();
                    if (usbListener != null) {
                        usbListener.removeUsb(device_remove);
                    }
                }
                Log.d(TAG, "onReceive: device_remove");
                break;
        }
    }
