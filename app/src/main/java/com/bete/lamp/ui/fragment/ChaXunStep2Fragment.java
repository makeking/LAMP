...
//    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {/* Inflate the layout for this fragment*/
        ...
        else item_jieguo.setText("结果(" + g_chaxun_pcrProject.project_danwei + ")");
    ...
    public void readJiLuDir() {
        ...
        g_chaxun_softcode = checkState.version;/*暂时无用*/
        g_chaxun_checkstate_timestr = checkState.checktime;
        g_chaxun_blockno = checkState.blockno;/*暂时无用*/
        g_chaxun_caijicount = checkState.caijicount;/*暂时无用*/
        ...
        for (int x = 0; x < GUANGNUM; x++) {
            ...
            for (int y = 0; y < BLOCKSIZE; y++) {
                ...
                for (int l = 0; l < g_pcrDataHandleRef.g_huadong_lvbo_cnt; l++) {
                    g_anaAlgorithm.KdsptForwardMBackN(g_chaxun_indexs_data, g_chaxun_fenxilvbodatas[x][y], g_chaxun_fenxiguiyidatas[x][y], g_pcrDataHandleRef.g_huadong_lvbo_num / 2, g_pcrDataHandleRef.g_huadong_lvbo_num / 2);//分析
                    g_anaAlgorithm.KdsptForwardMBackN_New(g_chaxun_indexs_data, g_chaxun_showlvbodatas[x][y], g_chaxun_showguiyidatas[x][y], g_pcrDataHandleRef.g_huadong_lvbo_num / 2, g_pcrDataHandleRef.g_huadong_lvbo_num / 2);//分析
                    System.arraycopy(g_chaxun_fenxiguiyidatas[x][y], 0, g_chaxun_fenxilvbodatas[x][y], 0, g_chaxun_indexs_data);
                    System.arraycopy(g_chaxun_showguiyidatas[x][y], 0, g_chaxun_showlvbodatas[x][y], 0, g_chaxun_indexs_data);
                }
...
//            getAntoThresvalue();
    ...
    public void savaDataToDir(String srcdir, String desdir) {
        ...
        csvfilebuff.delete(0, csvfilebuff.length()).append("项目名称," + g_chaxun_pcrProject.project_name + ", ," + g_chaxun_pcrProject.project_lot + ", , , ,\r\n");
        if (g_chaxun_pcrProject.project_type == GlobalDate.ProjectType.dingxing)
            csvfilebuff.append("分析方法," + "定性" + ", ," + g_chaxun_pcrProject.project_lot + ", , , ,\r\n");
        else
            csvfilebuff.append("分析方法," + "定量" + ", ," + g_chaxun_pcrProject.project_lot + ", , , ,\r\n");
        csvfilebuff.append("扩增程序," + "目标温度(℃)" + ",保持时间(min)," + "检测间隔(S)" + ",是否检测, , ,\r\n");
        for (int i = 0; i < 3; i++)
            if (g_chaxun_pcrProject.pcrLiuChengCanShuItems.get(i).isRead)
                csvfilebuff.append(String.valueOf(i+1) + "," + g_chaxun_pcrProject.pcrLiuChengCanShuItems.get(i).targetTemp + "," + g_chaxun_pcrProject.pcrLiuChengCanShuItems.get(i).holdTime + "," + g_chaxun_pcrProject.pcrLiuChengCanShuItems.get(i).intervalTime + ",是, , ,\r\n");
            else
                csvfilebuff.append(String.valueOf(i+1) + "," + g_chaxun_pcrProject.pcrLiuChengCanShuItems.get(i).targetTemp + "," + g_chaxun_pcrProject.pcrLiuChengCanShuItems.get(i).holdTime + "," + g_chaxun_pcrProject.pcrLiuChengCanShuItems.get(i).intervalTime + ",否, , ,\r\n");
        csvfilebuff.append("检测时间," + g_chaxun_checkstate_timestr + ", ," + "" + ", , , ,\r\n");
        if (g_chaxun_pcrProject.project_type == GlobalDate.ProjectType.dingxing)
            csvfilebuff.append("样本编号," + "孔位" + ",通道," + "靶标名称" + ",检测值,结果,,\r\n");
        else
            csvfilebuff.append("样本编号," + "孔位" + ",通道," + "靶标名称" + ",检测值,结果," + g_chaxun_pcrProject.project_danwei + ",\r\n");
            ...
            printThread = new Thread() {
                ...
                public void run() {
                    ...
                    for (int i = 0; i < BLOCKSIZE; i++) {
                        ...
                        String check_project_name = "检测项目:" + g_chaxun_pcrProject.project_name;
                        String check_project_lot = "批号:" + g_chaxun_pcrProject.project_lot;
                        ...
                        printbuff += "\n";
                        String check_project_sampleno = "样本编号:" + g_chaxun_samplenos[i];
                        String check_project_kongwei = "反应孔:" + String.valueOf((char) ('A' + g_chaxun_blockno)) + String.valueOf(i + 1);
                        ...
                        printbuff += dispalytemp;
                        printbuff += "\n";
                        printbuff += "检测时间:" + g_chaxun_checkstate_timestr + "\n";
                        printbuff += "检测结果:" + "\n";
                        for (int j = 0; j < GUANGNUM; j++) {
                            if (g_chaxun_pcrProject.project_item_ables[j]) {
                                if (g_chaxun_qcenable) {
                                    if (g_chaxun_pcrProject.project_type == GlobalDate.ProjectType.dingxing) {
                                        if (i > 1) {
                                            if (g_chaxun_lamp_resultitem[j][i].jieguo.equals("阳性")) {
                                                if(g_chaxun_lamp_resultitem[j][i].ct==0x0ffff)
                                                {
                                        ...
                                        if (i > 2) {
                                            if ((g_chaxun_lamp_resultitem[j][i].jieguo.equals("阳性")) && (g_chaxun_lamp_resultitem[j][i].jieguo.equals("无效")))
                                                printbuff += g_chaxun_lamp_resultitem[j][i].projectname + ":" + g_chaxun_lamp_resultitem[j][i].jieguo + "\n";
                                            else
    ...
    public void goDaoChuHandle() {
        ...
        if (!isUpanExist(getActivity())) {
            ...
            showProgressDialog("拷贝中");/*savedata();*/
            savaDataToDir(Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + Canstant.INNER_DATA_DIR + g_chaxun_checkstate_timestr, Environment.getExternalStorageDirectory().getAbsolutePath() + File.separator + Canstant.INNER_DATA_DIR + g_chaxun_checkstate_timestr);
                    ...
                    if (g_chaxun_pcrProject.project_item_ables[i]) {
                        if (g_chaxun_pcrProject.project_neican_tongdao != i + 1) {
                            if (g_chaxun_ct[i][0] > g_chaxun_pcrProject.project_babiaos[i]) {
                                qcright = false;
                                g_chaxun_lamp_resultitem[i][0].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 0 + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 0 + 1, "阳对", g_chaxun_pcrProject.project_item_names[i], "无效", g_chaxun_ct[i][0]);
                                LogUtils.d("阳性质控无效");
                            } else {
                                g_chaxun_lamp_resultitem[i][0].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 0 + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 0 + 1, "阳对", g_chaxun_pcrProject.project_item_names[i], String.valueOf(g_chaxun_ct[i][0]), g_chaxun_ct[i][0]);
                                LogUtils.d("阳性质控有效，报Tt值");
                            }

                            if (g_chaxun_ct[i][1] < g_chaxun_pcrProject.project_ncs[i]) {
                                qcright = false;
                                g_chaxun_lamp_resultitem[i][1].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 1 + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 1 + 1, "阴对", g_chaxun_pcrProject.project_item_names[i], "无效", g_chaxun_ct[i][1]);
                                LogUtils.d("阴性质控无效");
                            } else {
                                if (g_chaxun_pcrProject.project_neican_tongdao != 0)//有内参
                                {
                                    if (g_chaxun_ct[g_chaxun_pcrProject.project_neican_tongdao - 1][1] <= g_chaxun_pcrProject.project_neican) {
                                        g_chaxun_lamp_resultitem[i][1].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 1 + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 1 + 1, "阴对", g_chaxun_pcrProject.project_item_names[i], "有效", g_chaxun_ct[i][1]);
                                        LogUtils.d("阴性质控有效");
                                    } else {
                                        qcright = false;
                                        g_chaxun_lamp_resultitem[i][1].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 1 + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 1 + 1, "阴对", g_chaxun_pcrProject.project_item_names[i], "无效", g_chaxun_ct[i][1]);
                            ...
                            if (g_chaxun_ct[i][2] < g_chaxun_pcrProject.project_item_refs[i]) {
                                g_chaxun_lamp_resultitem[i][2].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 2 + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 2 + 1, g_chaxun_samplenos[2], g_chaxun_pcrProject.project_item_names[i], "阳性", g_chaxun_ct[i][2]);
                                LogUtils.d("样本阳性");
                            } else {
                                if (g_chaxun_pcrProject.project_neican_tongdao != 0)//有内参
                                {
                                    if (g_chaxun_ct[g_chaxun_pcrProject.project_neican_tongdao - 1][2] >= g_chaxun_pcrProject.project_neican) {
                                        g_chaxun_lamp_resultitem[i][2].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 2 + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 2 + 1, g_chaxun_samplenos[2], g_chaxun_pcrProject.project_item_names[i], "无效", g_chaxun_ct[i][2]);
                                        LogUtils.d("样本无效");
                                    } else {
                                        g_chaxun_lamp_resultitem[i][2].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 2 + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 2 + 1, g_chaxun_samplenos[2], g_chaxun_pcrProject.project_item_names[i], "阴性", g_chaxun_ct[i][2]);
                                        LogUtils.d("样本阴性");
                                    }
                                } else {
                                    g_chaxun_lamp_resultitem[i][2].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 2 + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 2 + 1, g_chaxun_samplenos[2], g_chaxun_pcrProject.project_item_names[i], "阴性", g_chaxun_ct[i][2]);
                            ...
                            if (g_chaxun_ct[i][3] < g_chaxun_pcrProject.project_item_refs[i]) {
                                g_chaxun_lamp_resultitem[i][3].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 3 + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 3 + 1, g_chaxun_samplenos[3], g_chaxun_pcrProject.project_item_names[i], "阳性", g_chaxun_ct[i][3]);
                                LogUtils.d("样本阳性");
                            } else {
                                if (g_chaxun_pcrProject.project_neican_tongdao != 0)//有内参
                                {
                                    if (g_chaxun_ct[g_chaxun_pcrProject.project_neican_tongdao - 1][3] >= g_chaxun_pcrProject.project_neican) {
                                        g_chaxun_lamp_resultitem[i][3].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 3 + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 3 + 1, g_chaxun_samplenos[3], g_chaxun_pcrProject.project_item_names[i], "无效", g_chaxun_ct[i][3]);
                                        LogUtils.d("样本无效");
                                    } else {
                                        g_chaxun_lamp_resultitem[i][3].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 3 + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 3 + 1, g_chaxun_samplenos[3], g_chaxun_pcrProject.project_item_names[i], "阴性", g_chaxun_ct[i][3]);
                                        LogUtils.d("样本阴性");
                                    }
                                } else {
                                    g_chaxun_lamp_resultitem[i][3].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 3 + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 3 + 1, g_chaxun_samplenos[3], g_chaxun_pcrProject.project_item_names[i], "阴性", g_chaxun_ct[i][3]);
                        ...
                        if (g_chaxun_pcrProject.project_item_ables[i]) {
                            if (g_chaxun_pcrProject.project_neican_tongdao != i + 1) {
                                if (g_chaxun_ct[i][j] < g_chaxun_pcrProject.project_item_refs[i]) {
                                    g_chaxun_lamp_resultitem[i][j].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + j + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + j + 1, g_chaxun_samplenos[j], g_chaxun_pcrProject.project_item_names[i], "阳性", g_chaxun_ct[i][j]);
                                    LogUtils.d("样本阳性");
                                } else {
                                    if (g_chaxun_pcrProject.project_neican_tongdao != 0)//有内参
                                    {
                                        if (g_chaxun_ct[g_chaxun_pcrProject.project_neican_tongdao - 1][j] >= g_chaxun_pcrProject.project_neican) {
                                            g_chaxun_lamp_resultitem[i][j].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + j + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + j + 1, g_chaxun_samplenos[j], g_chaxun_pcrProject.project_item_names[i], "无效", g_chaxun_ct[i][j]);
                                            LogUtils.d("样本无效");
                                        } else {
                                            g_chaxun_lamp_resultitem[i][j].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + j + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + j + 1, g_chaxun_samplenos[j], g_chaxun_pcrProject.project_item_names[i], "阴性", g_chaxun_ct[i][j]);
                                            LogUtils.d("样本阴性");
                                        }
                                    } else {
                                        g_chaxun_lamp_resultitem[i][j].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + j + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + j + 1, g_chaxun_samplenos[j], g_chaxun_pcrProject.project_item_names[i], "阴性", g_chaxun_ct[i][j]);
                    ...
                    if (g_chaxun_pcrProject.project_item_ables[i]) {
                        if (g_chaxun_pcrProject.project_neican_tongdao != i + 1) {
                            ...
                            if ((Math.abs(pcnongdu - g_chaxun_pcrProject.project_babiaos[i]) <= 0.4) && (Math.abs(biaonongdu - g_chaxun_pcrProject.project_dingliang_biaozhunnongdu) <= 0.4)) {
                                g_chaxun_lamp_resultitem[i][0].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 0 + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 0 + 1, "阳对", g_chaxun_pcrProject.project_item_names[i], "有效", g_chaxun_ct[i][0]);
                                LogUtils.d("PC质控有效");
                                g_chaxun_lamp_resultitem[i][2].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 2 + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 2 + 1, "标品", g_chaxun_pcrProject.project_item_names[i], "有效", g_chaxun_ct[i][2]);
                                LogUtils.d("标品质控有效");
                                ...
                                g_chaxun_lamp_resultitem[i][0].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 0 + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 0 + 1, "阳对", g_chaxun_pcrProject.project_item_names[i], "有效", g_chaxun_ct[i][0]);
                                LogUtils.d("PC质控有效");
                                g_chaxun_lamp_resultitem[i][2].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 2 + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 2 + 1, "标品", g_chaxun_pcrProject.project_item_names[i], "无效", g_chaxun_ct[i][2]);
                                LogUtils.d("标品质控无效");
                            } else if ((Math.abs(pcnongdu - g_chaxun_pcrProject.project_babiaos[i]) > 0.4) && (Math.abs(biaonongdu - g_chaxun_pcrProject.project_dingliang_biaozhunnongdu) < 0.4)) {
                                qcright = false;
                                g_chaxun_lamp_resultitem[i][0].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 0 + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + 0 + 1, "阳对", g_chaxun_pcrProject.project_item_names[i], "无效", g_chaxun_ct[i][0]);
                            ...
                            if (g_chaxun_pcrProject.project_neican_tongdao != i + 1) {
                                if (g_chaxun_ct[i][j] < g_chaxun_pcrProject.project_item_refs[i]) {
                                    ...
                                    if (g_chaxun_pcrProject.project_neican_tongdao != 0)//有内参
                                    {
                                        if (g_chaxun_ct[g_chaxun_pcrProject.project_neican_tongdao - 1][j] >= g_chaxun_pcrProject.project_neican) {
                                            g_chaxun_lamp_resultitem[i][j].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + j + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + j + 1, g_chaxun_samplenos[j],//
                                                    g_chaxun_pcrProject.project_item_names[i], "无效", g_chaxun_ct[i][j]);
                                            LogUtils.d("样本无效");
                                        } else {
                                            g_chaxun_lamp_resultitem[i][j].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + j + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + j + 1, g_chaxun_samplenos[j],//
                                                    g_chaxun_pcrProject.project_item_names[i], "阴性", g_chaxun_ct[i][j]);
                                            LogUtils.d("样本阴性");
                                        }
                                    } else {
                                        g_chaxun_lamp_resultitem[i][j].setLampResultItem(g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + j + 1, g_chaxun_blockno * GUANGNUM * BLOCKSIZE + i * BLOCKSIZE + j + 1, g_chaxun_samplenos[j],//
